...
- [x] 在 `core/paperkit-utils.sty` 的 cleveref 块中补充 `\crefrangeformat` 和 `\crefmultiformat`
- [x] 覆盖 figure / table / equation / theorem / lemma / definition（中英双语）
- [x] 示例效果：`\cref{fig:a,fig:b,fig:c}` → "图~(1, 2, 3)"；`\crefrange{fig:a}{fig:c}` → "图~(1~3)"

### Task 8.2 — 添加图表标题样式
- [x] 在 `core/paperkit-base.sty` 末尾添加 `\captionsetup`（中文模板下）：
  - figure: 宋体小四加粗，hang 缩进，标题放底部
  - table: 宋体小四加粗，hang 缩进，标题放顶部
- [x] 英文模板保持原有 caption 样式不变

### Task 8.3 — 添加 `\keywords` 关键词命令
- [x] 在 `core/paperkit-utils.sty` 添加 `\newcommand{\keywords}[1]`（基于 `\ifpkuLangZh` 显示"关键词"或"Keywords"）
- [x] 格式：`{\noindent\heiti 关键词：}~{#1}`（中文）/ `{\noindent\bfseries Keywords:}~{#1}`（英文）

### Task 8.4 — 优化列表环境间距
- [x] 在 `core/paperkit-base.sty` 添加 `\setlist` 全局配置（需 `\RequirePackage{enumitem}`）
- [x] 参数：`topsep=0.3em, itemsep=0ex plus 0.1ex, leftmargin=1.5em`
- [x] 中文论文列表紧凑，不浪费版面对

### Task 8.5 — CUMCM 模板中文节号
- [x] 在 `templates/cumcm/main_cumcm.tex` 的 preamble 区添加：
  ```latex
  \renewcommand{\thesection}{\chinese{section}、}
  ```
  使一级标题显示为"一、问题重述"而非"1 问题重述"
- [x] 同时更新 `templates/wuyi/main_wuyi.tex` 同步此配置
- [x] 确保 MCM 模板不受影响
...
> 2026 年国赛与五一杯均要求提交「人工智能工具使用声明」页。
> 本阶段统一补充并确保 CUMCM / Wuyi 两套模板均包含此页面。

### Task 9.1 — CUMCM 模板 AI 使用声明页
- [x] 创建 `templates/cumcm/sections/ai_declaration.tex`，内容包含：
  - 页面标题「人工智能工具使用声明」
  - 填写区：工具名称、使用范围、使用详情（`\underline` 格式）
  - 承诺声明文字
  - `% TODO(director): 核对 2026 年国赛官方 AI 声明页措辞与格式`
- [x] 在 `main_cumcm.tex` 中 `\input{sections/cover}` 之后接入：
  ```latex
  \input{sections/ai_declaration}
  ```
  并添加注释引导（电子版提交可注释掉）

### Task 9.2 — Wuyi 模板 AI 声明页接入
- [x] 检查 `templates/wuyi/sections/ai_declaration.tex` 已存在
- [x] 将 `main_wuyi.tex` 中的 `\input{sections/ai_declaration}` 从注释改为启用
- [x] 添加注释引导（赛方不要求可注释掉）

---

## Phase 10: 北京大学生数学建模与计算机应用竞赛模板

> 该赛事 (简称「北京赛」) 与国赛格式高度相似，可照搬 CUMCM 模板结构后微调。
> 需要差异化的部分：封面承诺书措辞、编号页头衔、赛事标题。

### Task 10.1 — 创建北京赛模板骨架
- [x] 复制 `templates/cumcm/` 为 `templates/beijing/`
- [x] 将 `main_cumcm.tex` 重命名为 `main_beijing.tex`
- [x] 批量替换：
  - `main_beijing.tex`、`cover.tex` 中的「全国大学生数学建模竞赛」→「北京大学生数学建模与计算机应用竞赛」
  - 「国赛/高教杯」→「北京赛」
  - `\cumcm*` token 前缀 → `\bjcontest*`（如 `\cumcmTihao` → `\bjcontestTihao`）
- [x] 添加 `% TODO(director): 核对北京赛官方承诺书格式与措辞`
- [x] 确保所有 `\input{sections/...}` 路径正确

### Task 10.2 — 注册到构建系统
- [x] 在 `scripts/build.py` 的 `TARGETS` 字典中添加 `beijing` 条目：
  ```python
  "beijing": {
  ...
  ```
- [x] 在 `scripts/new_contest.py` 的模板列表中添加 `beijing`
- [x] 编译验证：`python scripts/build.py --target beijing`
